...
class Track:
    ...
    def search(self, val):
        ...
        while left < right:
            mid = (left + right) // 2
            if self.sorted_list[mid] < val:
                left = mid + 1
            elif self.sorted_list[mid] > val:
                right = mid - 1
            else:
                left = mid
                break
        return -1 if self.sorted_list[left] == val else left

    def insert(self, idx, val):
        if self.sorted_list[idx] < val:
            self.sorted_list.insert(idx+1, val)
        else:
        ...
        if self.overflow:
            ...
            while left < right:
                ...
                if self.sorted_list[mid] < val:
                    ...
                    left = mid
                    break
            self.sorted_list = self.sorted_list[:left] + self.sorted_list[left+1:]
            return
        if len(self.sorted_list) == self.max_length:
            self.overflow = True
...
def make_room_key(room):
    val = 0
    col = 1
    for y in range(1, 11):
        for x in range(1, 17):
            val += int(room[y][x])*col
            col *= 8
    return val
...
def solve(room, depth):
    ...
    if room_count == MAX_ROOM_COUNT:
        giveup = True
        return True
    room_key = make_room_key(paint(room, find_player(room), 7))
    idx = track.search(room_key)
    if idx < 0:
        return False
    track.insert(idx, room_key)
    if is_dead(room):
        return False
